...
void conv_tile(const float* in, float* out, const float* ker,
    ...
    const int in_stride = W + 2 * p;
      for (int ty = 0; ty < H; ty += TILE) { //H and W are divided into TILE X TILE blocks hence both ty and tx jump by tile size.
       
            int oy_end = std::min(ty + TILE, H); //tile size might not divide H and W evenly, so in such a case, take the min of ty/tx + tile and the row/col number.
